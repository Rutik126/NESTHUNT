...
const RoomOwnerProfile = () => {
  const getImageUrl = (imgPath) => {
    ...
    return `${import.meta.env.VITE_BACKEND_BASEURL}/${imgPath.replace(/^\/+/, '')}`;
  };
  ...
  const toggleVacancy = async (propertyId, currentVacantStatus) => {
    try {
      ...
      const response = await axios.patch(
        `${import.meta.env.VITE_BACKEND_BASEURL}/api/properties/${propertyId}/vacancy`,
      ...
      if (response.data.success) {
        ...
        const updatedProperties = await axios.get(
          `${import.meta.env.VITE_BACKEND_BASEURL}/api/properties/my-properties`,
          {
    ...
    const fetchData = async () => {
      try {
        ...
        const [profileRes, propertiesRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_BACKEND_BASEURL}/api/roomowner/dashboard`, {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          }),
          axios.get(`${import.meta.env.VITE_BACKEND_BASEURL}/api/properties/my-properties`, {
            headers: { Authorization: `Bearer ${token}` },
            withCredentials: true,
          })
        ]);
  ...
  const handlePropertySubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_BASEURL}/api/properties`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <img
        src={
          roomOwner.profilePhoto
            ? `${import.meta.env.VITE_BACKEND_BASEURL}/${roomOwner.profilePhoto.replace(/\\/g, '/')}`
    ...
    <div className="manage-properties-page">
      ...
      {properties.length === 0 ? (
        ...
        properties.map((property) => {
          ...
          (images[0].startsWith("http") ? images[0] : `${import.meta.env.VITE_BACKEND_BASEURL}${images[0]}`) 
