    ...
    const fetchData = async () => {
      try {
        ...
        if (token && storedUserType === 'user') {
          const profile = await axios.get(`${import.meta.env.VITE_BACKEND_BASEURL}/api/user/profile`, {
        ...
        const propsResponse = await axios.get(`${import.meta.env.VITE_BACKEND_BASEURL}/api/properties/`, {
          params: { limit: 20 },
          timeout: 5000
        });
  ...
  const contactOwner = async (propertyId) => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_BASEURL}/api/properties/${propertyId}/owner`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  const saveProperty = async (propertyId) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_BASEURL}/api/user/save-property`,
        { propertyId },
        {
          headers: { Authorization: `Bearer ${token}` }
        }
